...
class ByteStream {
  constructor(canvas) {
    ...
    // Initialize spiral stream
    for (let i = 0; i < this.length; i++) {
      this.stream.push({
        char: this.chars[Math.floor(Math.random() * this.chars.length)],
        angle: this.angle + i * this.angleIncrement,
        radius: i * this.radiusIncrement
      });
    }
  }
  ...
  update() {
    ...
    for (let i = 0; i < this.stream.length; i++) {
      this.stream[i].radius += this.speed;
      this.stream[i].angle += this.angleIncrement * 0.1;
      
      // Reset characters that spiral too far out
      if (this.stream[i].radius > Math.max(this.width, this.height) * 0.6) {
        this.stream[i].radius = 0;
        this.stream[i].angle = this.angle + i * this.angleIncrement;
      }
    }
  }
  ...
  draw() {
    ...
    for (let i = 0; i < this.stream.length; i++) {
      ...
      const x = this.centerX + Math.cos(char.angle) * char.radius;
      const y = this.centerY + Math.sin(char.angle) * char.radius;
      
      // Fading effect based on radius
      const alpha = Math.min(1, (char.radius / 100) * this.opacity);
